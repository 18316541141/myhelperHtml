<template>
  <div>
    <el-dialog
      title="导出excel列表"
      v-bind:visible.sync="exportDialog"
        ...
        <el-table-column property="index" label="操作" width="100px">
          <template slot-scope="scope">
            <el-button
              ...
              @click="exportExcel(scope.row.index,scope.row.excelType)"
            >导出</el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-table
      v-bind:data="retData_.data.pageDataList"
      style="width: 100%"
      size="mini"
      v-bind:height="tableHeight_"
      v-bind:select-on-indeterminate="true"
      v-bind:border="true"
    ...
    <el-pagination
      v-if="retData_.data.totalItemCount>0"
      v-bind:current-page="retData_.data.currentPageIndex"
      v-bind:total="retData_.data.totalItemCount"
      layout="sizes, prev, pager, next, jumper, ->, total, slot"
      v-bind:page-size="retData_.data.pageSize"
      v-bind:background="true"
      v-bind:page-count="retData_.data.totalPageCount"
      v-bind:page-sizes="[20,40,60,80,100]"
      v-on:size-change="sizeChange"
      v-on:current-change="currentChange"
      v-on:prev-click="currentChange"
      v-on:next-click="currentChange"
    ></el-pagination>
  </div>
</template>
<script>
    ...
    window.addEventListener(
      ...
      function() {
        if (thiz.tableHeight === undefined) {
          thiz.tableHeight_ =
            window.innerHeight - 148 - thiz.reduceHeight + "px";
        }
      },
      false
    );
