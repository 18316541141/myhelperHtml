...
export default {
  ...
  methods: {
    ...
    crop() {
      var thiz = this;
      this.$post(
        this.$store.state.PROXY+"/index/singleImageCrop",
        {
        ...
        function(result) {
          ...
          thiz.src =
            thiz.$store.state.PROXY+"/index/showImage?imgName=" +
            thiz.imgName_;
        }
      );
    }
  ...
  mounted() {
    ...
    new this.$WebUploader.Uploader({
      ...
      auto: true, //选中文件后自动上传
      server: thiz.$store.state.PROXY+"/index/uploadSingleImage", //处理上传文件的统一控制器
      ...
      .on(
        ...
        uploadCallback(this, function(file, response) {
          if (response.code === 0) {
            ...
            thiz.$emit("update:imgName", data.imgName);
            thiz.$emit("update:thumbnailName", data.thumbnailName);
            thiz.src = thiz.$store.state.PROXY+"/index/showImage?imgName=" + thiz.imgName_;
            if (thiz.cut === true) {
              thiz.$openLoading();
